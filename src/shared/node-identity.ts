...
export function getNodeName(): string {
  ...
  try {
    ...
    if ((settings as any).CLAUDE_MEM_NODE_NAME) {
      cachedNodeName = (settings as any).CLAUDE_MEM_NODE_NAME;
      return cachedNodeName!;
    }
  } catch (error) {
...
export function getInstanceName(): string {
  ...
  try {
    ...
    const settingsPath = path.join(dataDir, 'settings.json');
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
    return (settings as any).CLAUDE_MEM_INSTANCE_NAME || '';
  } catch (error) {
...
export function getNetworkMode(): 'standalone' | 'server' | 'client' {
  ...
  try {
    ...
    const settingsPath = path.join(dataDir, 'settings.json');
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
    const mode = (settings as any).CLAUDE_MEM_NETWORK_MODE;
...
export function getLlmSource(): string {
  ...
  try {
    ...
    const settingsPath = path.join(dataDir, 'settings.json');
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
    if ((settings as any).CLAUDE_MEM_LLM_SOURCE) {
      return (settings as any).CLAUDE_MEM_LLM_SOURCE;
    }
  } catch (e) {
